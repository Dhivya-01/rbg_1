...
const BentoGridItem = ({
  ...
  return (
    <motion.div
      ...
      <div
        className="absolute top-0 -right-1/2 z-0 size-full bg-[linear-gradient(to_right,#3d16165e_1px,transparent_1px),linear-gradient(to_bottom,#3d16165e_1px,transparent_1px)] bg-[size:24px_24px] [mask-image:radial-gradient(ellipse_60%_50%_at_50%_0%,#000_70%,transparent_100%)]" />
      {/* Big background icon */}
      <div
        className="absolute right-2 bottom-3 text-primary/5 group-hover:text-primary/10 z-0 scale-[6] transition-all duration-700 group-hover:scale-[6.5]">
      ...
      <div className="relative z-10 flex flex-col justify-between h-full">
        <div>
          <div
            className="mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-primary/10 text-primary shadow transition-all duration-500 group-hover:bg-primary/20">
            <motion.div variants={iconVariants} initial="initial" animate="animate">
              {icon}
            </motion.div>
          </div>
      ...
      {/* Bottom glow */}
      <div
        className="absolute bottom-0 left-0 w-full h-1 bg-gradient-to-r from-primary to-primary/30 blur-2xl transition-all duration-500 group-hover:blur-lg" />
    </motion.div>
  );
};
